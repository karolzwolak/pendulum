...
class CartPoleSimulation(Simulation):
    def __init__(
        ...
        super().__init__(draw_options, cart_mass=cart_mass)
        ...
        self.bob_body.position = (
            self.cart_body.position.x + length *
            pymunk.Vec2d(1, 0).rotated(angle).x,
            self.cart_body.position.y + length *
            pymunk.Vec2d(1, 0).rotated(angle).y,
        )
